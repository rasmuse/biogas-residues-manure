...
@cli.command()
@click.option('--data', '-d', type=click.File(mode='r'), default='-')
@click.option('--key-property', '-k', required=True, type=str)
@click.argument('src', type=click.Path(exists=True))
@click.argument('dst', type=click.Path(exists=False))
def addfield(data, key_property, src, dst):
    data = json.loads(data.read())
    data = pd.Series(data)
    write_data_to_regions(src, dst, key_property, data)
